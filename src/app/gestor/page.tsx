  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F1F5F9' }}>
      ...
      <main style={{ maxWidth: 1000, margin: '0 auto', padding: '20px 16px' }}>
        ...
        {tab === 'parceiros' && (
          <div style={{ background:'#fff', borderRadius:14, border:'1px solid #E2E8F0', overflow:'hidden' }}>
            <div style={{ display:'grid', gridTemplateColumns:'1.8fr 1fr 1.1fr 90px 110px', gap:8, padding:'10px 16px', background:'#F8FAFC', borderBottom:'1px solid #E2E8F0' }}>
              {['Nome','Especialidade','Contato','Cadastro','QR Code'].map(h => (
                <div key={h} style={{ fontSize:11, fontWeight:600, color:'#94A3B8', textTransform:'uppercase', letterSpacing:'.05em' }}>{h}</div>
              ))}
            </div>
            ...
            {parceiros.map((p, i) => (
              <div key={p.id} style={{ display:'grid', gridTemplateColumns:'1.8fr 1fr 1.1fr 90px 110px', gap:8, padding:'12px 16px', borderBottom: i < parceiros.length-1 ? '1px solid #F1F5F9' : 'none', alignItems:'center' }}>
                <div>
        ...
        <div onClick={() => setQrModal(null)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.6)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:100, padding:20 }}>
          <div onClick={e => e.stopPropagation()} style={{ background:'#fff', borderRadius:18, padding:'24px', maxWidth:540, width:'100%', boxShadow:'0 20px 60px rgba(0,0,0,.25)' }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}>
              ...
              <button onClick={() => setQrModal(null)} style={{ background:'none', border:'none', fo